    ...
    drumGrid(p), pianoRoll(p)
{
    ...
    lockToBpmLbl.setImage(loadSkin("lockBpmLbl.png")); addAndMakeVisible(lockToBpmLbl);
...
void BoomAudioProcessorEditor::resized()
{
    ...
    bpmLbl.setBounds(S(25, 15, 121, 49));
    bpmLockChk.setBounds(S(25, 15, 34, 34));
    lockToBpmLbl.setBounds(S(20, 15, 147, 26));
    ...
    // DRUM GRID (main window) — exact size
    {
        ...
        drumGrid.setSize(gridArea.getWidth() * 2, gridArea.getHeight() * 2);
    }
...
void AIToolsWindow::resized()
{
    ...
    selectAToolLbl.setBounds(S(550, 50, 140, 40));
    lockToBpmLbl.setBounds(S(20, 20, 120, 40));
    bpmLbl.setBounds(S(20, 60, 120, 40));

    // ---- Rhythmimick row (top block) ----
    int y_offset = 130;
    rhythmimickLbl.setBounds(S(300, y_offset, 240, 40));
    toggleRhythm.setBounds(S(550, y_offset, 120, 40));
    rhythmimickDescLbl.setBounds(S(300, y_offset + 35, 250, 40));
    
    recordUpTo60LblTop.setBounds(S(50, y_offset + 50, 180, 20));
    btnRec1.setBounds(S(50, y_offset + 20, 30, 30));
    playBtn.setBounds(S(90, y_offset + 20, 30, 30));
    rhythmSeek.setBounds(S(130, y_offset + 20, 140, 30));
    btnStop1.setBounds(S(130, y_offset + 20, 30, 30));

    btnGen1.setBounds(S(50, y_offset + 80, 90, 30));
    btnSave1.setBounds(S(150, y_offset + 80, 90, 30));
    btnDrag1.setBounds(S(250, y_offset + 80, 90, 30));

    // ---- Slapsmith row ----
    y_offset = 270;
    slapsmithLbl.setBounds(S(300, y_offset, 240, 40));
    toggleSlap.setBounds(S(550, y_offset, 120, 40));
    slapsmithDescLbl.setBounds(S(300, y_offset + 35, 250, 40));
    slapSmithGridLbl.setBounds(S(50, y_offset - 20, 180, 20));
    miniGrid.setBounds(S(50, y_offset, 250, 80));
    

    btnGen2.setBounds(S(50, y_offset + 90, 90, 30));
    btnSave2.setBounds(S(150, y_offset + 90, 90, 30));
    btnDrag2.setBounds(S(250, y_offset + 90, 90, 30));

    // ---- Style Blender row ----
    y_offset = 410;
    styleBlenderLbl.setBounds(S(300, y_offset, 240, 40));
    toggleBlend.setBounds(S(550, y_offset, 120, 40));
    styleBlenderDescLbl.setBounds(S(300, y_offset + 35, 250, 40));

    // Style A/B combo boxes
    styleABox.setBounds(S(50, y_offset, 120, 28));
    styleBBox.setBounds(S(180, y_offset, 120, 28));

    // One blended slider between them
    blendSlider.setBounds(S(50, y_offset + 35, 250, 20)); // below the boxes; adjust to taste

    btnGen3.setBounds(S(50, y_offset + 90, 90, 30));
    btnSave3.setBounds(S(150, y_offset + 90, 90, 30));
    btnDrag3.setBounds(S(250, y_offset + 90, 90, 30));

    // Arrows between toggles and the text blocks (adjust Y to your art)
    arrowRhythm.setBounds(S(500, 145, 40, 20));
    arrowSlap.setBounds(S(500, 285, 40, 20));
    arrowBlend.setBounds(S(500, 425, 40, 20));
    arrowBeat.setBounds(S(500, 565, 40, 20));

    // ---- Beatbox row (bottom) ----
    y_offset = 550;
    beatboxLbl.setBounds(S(300, y_offset, 240, 40));
    toggleBeat.setBounds(S(550, y_offset, 120, 40));
    beatboxDescLbl.setBounds(S(300, y_offset + 35, 280, 40));

    recordUpTo60LblBottom.setBounds(S(50, y_offset + 50, 180, 20));
    btnRec4.setBounds(S(50, y_offset + 20, 30, 30));
	btnPlay4.setBounds(S(90, y_offset + 20, 30, 30));
    beatboxSeek.setBounds(S(130, y_offset + 20, 140, 30));
    btnStop4.setBounds(S(130, y_offset + 20, 30, 30));

    btnGen4.setBounds(S(50, y_offset + 90, 90, 30));
    btnSave4.setBounds(S(150, y_offset + 90, 90, 30));
    btnDrag4.setBounds(S(250, y_offset + 90, 90, 30));

    // ---- Home button ----
    btnHome.setBounds(S(590, 700, 80, 80));
}
...
void RollsWindow::resized()
{
    ...
    rollsTitleImg.setBounds(x - 50, y, targetW, targetH);
